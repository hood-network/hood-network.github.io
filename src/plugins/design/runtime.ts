...
export interface RuleCSS {
	...
	children: RuleCSS[];
	addToken(reference: TokenReference, value: string | number | undefined): void;
}
...
class TypescaleBuilder extends TypescaleBuilderBase<SystemTypescale> {
	emphasized(
		builderFn: (builder: TypescaleBuilderBase<TypescaleBase>) => TypescaleBase,
	) {
...
class DesignSerializer {
	serialize({ tokens, external }: Design) {
		...
		for (const [face, scale] of this.entries(tokens.typescale)) {
			if (face === "emphasized") {
				for (const [realFace, realScale] of this.entries(
					...
					this.flattenTypescale(root, realFace, realScale, ".emphasized");
				}
		...
		for (const [token, style] of this.entries(external.syntect)) {
			this.flattenColor(root, light, `external.syntect.${token}`, style.color);
	...
	flattenTypescale(
		...
		for (const [size, data] of this.entries(scale)) {
			root.addToken(`sys.typescale${prefix}.${face}.${size}.font`, data.font);

			root.addToken(`sys.typescale${prefix}.${face}.${size}.size`, data.size);
